...
const checkBucketExists = ( Bucket ) => new Promise((resolve, reject) => {
...
const getBucketObjects = ( Bucket) => new Promise((resolve, reject) => {
    s3.listObjectVersions({Bucket}, (err, data) => {
        ...
        resolve(versions.concat(deleteMarkers)); 
    });  
});
const removeAllObjects = (bucketObjects, bucket) => new Promise((resolve, reject) => {
        ...
        for (const bucket of s3ClearOptions.buckets) {
            try{
                if(await checkBucketExists(bucket)) {
                    serverless.cli.log(`${bucket} exists`);
                    const bucketObjects =  await getBucketObjects(bucket);
                    if (bucketObjects.length) {
                        await removeAllObjects(bucketObjects, bucket);
                    }
                    serverless.cli.log(`${bucket} empty`)
